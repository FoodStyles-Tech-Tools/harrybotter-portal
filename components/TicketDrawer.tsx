          ...
          >
            <div className="flex items-center justify-between p-8 border-b border-white/60 bg-white/40">
              <div className="flex flex-col gap-1">
                <span className="text-xs font-semibold text-blue-600 bg-blue-50 px-2 py-1 rounded-md self-start border border-blue-100 uppercase tracking-widest">
              ...
              <button
                onClick={onClose}
                className="p-2 rounded-xl text-gray-400 hover:text-gray-900 hover:bg-white transition-all focus:outline-none"
              >
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/40 rounded-2xl p-4 border border-white/60 shadow-sm">
                  <h3 className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-2">Reporter</h3>
                  <div className="flex items-center gap-3">
                    {ticket.reporterAvatar ? (
                      ...
                      <div className="w-7 h-7 rounded-full bg-blue-50 flex items-center justify-center text-[10px] font-medium text-blue-600">
                        {ticket.requestedBy?.charAt(0) || 'U'}
                      </div>
                    )}
                    <span className="text-sm font-medium text-gray-900 truncate">{ticket.requestedBy}</span>
                  </div>
                </div>
                <div className="bg-white/40 rounded-2xl p-4 border border-white/60 shadow-sm">
                  <h3 className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-2">Project</h3>
                  <p className="text-sm font-medium text-gray-900 truncate">{ticket.projectName || 'Default'}</p>
                </div>
              </div>

              <div className="bg-white/40 rounded-2xl p-5 border border-white/60 shadow-sm">
                <h3 className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-3">Description</h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/40 rounded-2xl p-4 border border-white/60 shadow-sm">
                   <h3 className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-2">Created</h3>
                   <p className="text-sm font-medium text-gray-900">{formatDateTime(ticket.created_at)}</p>
                </div>
                {dueDate && (
                  <div className="bg-white/40 rounded-2xl p-4 border border-white/60 shadow-sm">
                    <h3 className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-2">Deadline</h3>
              ...
              {ticket.status === 'Cancelled' && (() => {
                ...
                return cancellationReason ? (
                  <div className="bg-rose-50/50 rounded-2xl p-5 border border-rose-100 shadow-sm">
                    <h3 className="text-[10px] font-medium text-rose-500 uppercase tracking-wider mb-2">Cancellation Reason</h3>
              ...
              <div className="space-y-4">
                <h3 className="text-[10px] font-medium text-gray-400 uppercase tracking-wider ml-1">Timeline Details</h3>
                <div className="bg-white/40 rounded-2xl border border-white/60 shadow-sm overflow-hidden">
                  {ticket.assigned_at && (
                    <div className="flex items-center justify-between p-4 border-b border-white/40 last:border-0 hover:bg-white/50 transition-all">
                      <span className="text-[10px] font-medium text-gray-500 uppercase tracking-tight">Assigned At</span>
                      <span className="text-xs font-medium text-gray-900">{formatDateTime(ticket.assigned_at)}</span>
                    </div>
                  )}
                  {ticket.completed_at && (
                    <div className="flex items-center justify-between p-4 border-b border-white/40 last:border-0 hover:bg-white/50 transition-all">
                      <span className="text-[10px] font-medium text-gray-500 uppercase tracking-tight">Completed At</span>
              ...
              {ticket.relevantLink && (
                <div className="bg-white/40 rounded-2xl p-5 border border-white/60 shadow-sm">
                  <h3 className="text-[10px] font-medium text-gray-400 uppercase tracking-wider mb-3">Attachments</h3>
